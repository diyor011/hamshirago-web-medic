...
const DISMISS_KEY = "medic-push-prompt-dismissed-at";
...
export default function PushPermissionPrompt() {
  ...
  useEffect(() => {
    ...
    // Не показывать если уже разрешено или заблокировано
    if (Notification.permission !== "default") return;

    // Не показывать если не залогинен
    if (!localStorage.getItem("medic_token")) return;

    // Не показывать если недавно отложил
    ...
    <>
      ...
      <div style={{
        ...
        <p style={{ fontSize: 14, color: "#64748b", lineHeight: 1.55, marginBottom: 24 }}>
          Получайте новые заказы и сообщения от клиентов мгновенно — даже когда вкладка закрыта.
        </p>
